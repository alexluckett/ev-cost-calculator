...
import { Button, Disclosure, Field, NumberInput, Pill, Select, Toggle } from './ui';
...
export function VehicleCard({
  ...
  // the car is based on; the preset is only a source of defaults.
  ...
  return (
    <article className="vehicle-card" style={{ borderTopColor: scenario.colour }}>
      ...
      <Disclosure title="Vehicle specification" badge={preset ? 'Autofilled' : 'Custom'}>
        ...
        {electric ? (
          <div className="grid-3">
            <Field label="Usable battery" hint="Usable capacity, not the gross figure on the brochure. It sets what a full charge costs and how far it goes.">
              <NumberInput value={vehicle.usableBatteryKWh} min={0} step={0.5} suffix="kWh" onChange={(v) => setVehicle('usableBatteryKWh', v)} />
            </Field>
            <Field label="Efficiency" hint="Miles per kWh measured at the battery. Your car's trip computer shows this. It is the single most important number for an EV's running cost.">
              <NumberInput value={vehicle.miPerKWh} min={0.5} max={10} step={0.1} suffix="mi/kWh" onChange={(v) => setVehicle('miPerKWh', v)} />
            </Field>
            <Field label="Peak DC charging">
        ...
        {burnsFuel ? (
          <div className="grid-3">
            ...
            <Field label="Real-world consumption" hint="What you actually get, not the official combined figure. Most cars fall 10–20% short of their WLTP number.">
        ...
        {vehicle.fuelType === 'phev' ? (
          <div className="grid-2">
            ...
            <Field label="Share of miles on electricity" hint="The number that decides whether a plug-in hybrid is brilliant or pointless. Short commutes plugged in nightly can reach 70–80%; a company car never plugged in is 0%.">
        ...
        <div className="grid-3">
          <Field label="CO2 emissions" hint="Official combined figure. Drives the company car tax band and the road tax band for older cars.">
            <NumberInput value={vehicle.co2gPerKm} min={0} step={1} suffix="g/km" onChange={(v) => setVehicle('co2gPerKm', v)} />
          </Field>
          <Field label="List price when new" hint="Used for the road tax Expensive Car Supplement threshold of £40,000.">
            <NumberInput value={vehicle.listPriceGBP} min={0} step={500} prefix="£" onChange={(v) => setVehicle('listPriceGBP', v)} />
          </Field>
          <Field label="What you paid" hint="Used for depreciation when you own the car outright.">
      ...
      <Disclosure title="Standing costs">
        <div className="grid-2">
          ...
          <Field label="Servicing and maintenance" hint="Electric cars have no oil, spark plugs, cambelt, clutch or exhaust, so this is typically a third to a half of a comparable combustion car.">
          ...
          <Field label="Tyres and consumables" hint="Scales with mileage. Electric cars are heavier and more torquey, so they get through tyres faster — but they barely use their brakes.">
